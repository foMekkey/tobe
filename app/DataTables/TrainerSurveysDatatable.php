...
class TrainerSurveysDatatable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables($query)
            ...
            ->editColumn('action', 'trainer.surveys.action')
    ...
    public function query(Survey $model)
    {
        return $model->newQuery()->select('id', 'course_id', 'title', 'date', 'is_day_star', 'show_results_in_course')
                     ->where('user_id',auth()->user()->id)->where('course_id',$this->course_id);
    }
    ...
    {
        return 'TrainerSurveys_' . date('YmdHis');
    }
}
