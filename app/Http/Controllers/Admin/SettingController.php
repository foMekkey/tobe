...
class SettingController extends Controller
{
    ...
    public function index()
    {
        ...
        // إضافة إيميل استلام الإشعارات
        $notifications_email   = Setting::where('name', 'notifications_email')->first();
        if (!$notifications_email) {
            // إذا لم يكن موجودًا، قم بإنشائه
            $notifications_email = new Setting();
            $notifications_email->name = 'notifications_email';
            $notifications_email->value = '';
            $notifications_email->save();
            $notifications_email = $notifications_email->value;
        } else {
            $notifications_email = $notifications_email->value;
        }
        ...
        return view('backend.setting.setting', compact(
            ...
            'terms_of_service',
            'check_points_group',
            'notifications_email', // إضافة متغير إيميل الإشعارات
    ...
    public function UpdateFristTap(Request $request)
    {
        ...
        // تحديث إيميل الإشعارات
        $notifications_email = Setting::where('name', 'notifications_email')->first();
        if (!$notifications_email) {
            // إذا لم يكن موجودًا، قم بإنشائه
            $notifications_email = new Setting();
            $notifications_email->name = 'notifications_email';
            $notifications_email->value = $request->notifications_email;
            $notifications_email->save();
        } else {
            $notifications_email->value = $request->notifications_email;
            $notifications_email->save();
