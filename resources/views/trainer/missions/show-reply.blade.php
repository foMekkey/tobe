        ...
        <div class="kt-portlet__body">
            <form action="{{ route('TrainerMissionsUpdateReply',['id'=>$reply->id]) }}" method="post" id="kt_form_1">
                ...
                <div class="kt-portlet__body">
                    ...
                    @if ($reply->mission->file)
                        <div class="form-group row">
                            ...
                            <div class="col-lg-9 col-xl-9">
                                <a href="{{ asset("uploads/".$reply->mission->file) }}" download="">{{ __('pages.download') }}</a>
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="form-group row">
                        <label class="col-xl-3 col-lg-3 col-form-label">  {{ __('pages.status-column') }} <span style="color: red">*</span></label>
                        <div class="col-lg-9 col-xl-9">
                            <select class="form-control" name="status" >
                                <option value="1" @if($reply->status == '1')selected @endif> ينتظر المراجعة </option>
                                <option value="2" @if($reply->status == '2')selected @endif> تم الإنجاز </option>
                            </select>
                        </div>
                    </div>
                    
                    <div class="form-group row">
                        <label class="col-xl-3 col-lg-3 col-form-label">  {{ __('pages.trainer_rate') }} <span style="color: red">*</span></label>
                        <div class="col-lg-9 col-xl-9">
                            <div class="my-rating"></div>
                            <input name="trainer_rate" value="{{ $reply->trainer_rate ? $reply->trainer_rate : '' }}" id="add_review-rate" style="width: 0; border: none" tabindex="-1" />
                        </div>
                    </div>
