                    ...
                    @if($courseUser)
                        @foreach ($courseSurveys as $survey)
                            <a href="{{ route('showSurveysStudent', $survey->id) }}">
                                <img class="img-fluid" src="{{ asset('admin/assets') }}/{{ $survey->is_day_star ? 'day_star.jpg' : 'survey_icon.jpg' }}" style="margin-top: 10px;" />
                            </a>
                        @endforeach
                        @foreach ($courseCompletedSurveys as $survey)
                            <a href="{{ route('showSurveyResultsStudent', $survey->id) }}">
                                <img class="img-fluid" src="{{ asset('admin/assets/survey_results_icon.png') }}" style="margin-top: 10px;" />
                            </a>
                        @endforeach
                    @else
                        <a class="btn btn-brand btn-elevate btn-block join_course" href="#" style="margin-top: 20px;">{{ __('pages.got_course') }}</a>
                    @endif
                </div>
                            ...
                            <div class="course_subscribe">
                                <div class="course_info">
                                    ...
                                    <div style="margin-right: 5px;">
                                        <h4>المحاضر</h4>
                                        <h3>{{ ($courseData->user->f_name ?? '') . ' ' . ($courseData->user->l_name ?? '') }}</h3>
                                    </div>
                                </div>
                                <div class="course_info">
                                    <h4>تاريخ البدء</h4>
    ...
    <script>
        $('.join_course').click(function(event) {
            ...
            $.ajax({
                ...
                data: {'_token': "{{ csrf_token() }}", 'id': {{ $courseData->id }}},
