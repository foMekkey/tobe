            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="name_event" name="name" value="" required="" readonly="">
                    </div>

                    <div class="form-group">
                        <label for="recipient-name" class="form-control-label">{{ __('pages.event_start') }}:</label>
                        <input type="date" class="form-control" name="start_date_event" id="start_date_event" value="" required="" readonly="">
                    </div>

                    <div class="form-group">
                        <label for="recipient-name" class="form-control-label">{{ __('pages.event_end') }}:</label>
                        <input type="date" class="form-control" name="end_date_event" id="end_date_event" value="" required="" readonly="">
                    </div>
    ...
    <script>
        $(document).on('click', '.fc-content', function () {
            ...
            if (eventType == '1') {
                $.ajax({
                    type: "GET",
                    url: "{{ url('student/events/edit') }}" + '/' + id,
                    dataType: "json",
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ url('student/events/ajaxMeetingShow') }}" + '/' + id,
                    dataType: "json",
                    success: function (msg) {
                        ...
                        if (msg.meeting.join_url !== '') {
                            $('#meeting_join').html('<a class="btn btn-primary" href="' + msg.meeting.join_url + '" target="_blank">{{ __("pages.join") }}</a>');
        ...
        $("#appointment_edit").on('click', function (event) {
            ...
            $.ajax({
                type: "POST",
                url: "{{ url('student/events/postupdate') }}" + '/' + $('#event_id').val(),
        ...
        $("#appointment_delete").on('click', function (event) {
            ...
            $.ajax({
                type: "GET",
                url: "{{ url('student/events/delete') }}" + '/' + id,
                dataType: "json",
                success: function (msg) {
                    console.log(msg);
                    //   $("#event-name").val('');
